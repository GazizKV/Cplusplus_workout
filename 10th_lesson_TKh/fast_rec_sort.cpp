...
void	merge_sort(double *array, int16_t array_size)
{
	...
	//Merge these two already sorted parts array:
	...
	while (left_i < left_size)//coping left elements from the size (if there is left something)
	{
		buffer[buffer_i] = left[left_i];
		left_i += 1;
		buffer_i += 1;
	}
	while (right_i < right_size)//coping right elements from the size(if there is right something)
	{
		buffer[buffer_i] = right[right_i];
		right_i += 1;
		buffer_i += 1;
	...
	// coping from buffer to original array:
...
int	main()
{
	...
	{
		std::cerr << "Your array size unacceptable!\n";
		return 1;
	}
