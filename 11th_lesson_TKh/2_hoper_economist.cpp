...
int min_cost(int n, int price[])
{
	...
	while (current != 1)
	{
		if (cost[current - 1] == cost[current] - price[current])
			...
			throw -1; // Nonsence! I shold never achive this line!
		std::cout << current << " ";
	}
...
{
	int n;
	std::cout << "Hooper economist wants to go from point 1 to point n. Enter n:\n";
	std::cin >> n;
	if (n > 100) 
	{
	...
	int price[101];
	std::cout << "Enter prices of visitiing point (from point 1 to point n):\n";
	for (int i = 1; i <= n; i++)
	{
		std::cin >> price[i];
	}
	std::cout << "Min cost of achiving point n from poin 1 is " << min_cost(n, price) << '\n';
